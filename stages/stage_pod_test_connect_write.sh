...
# ----------
  ...
  generic_msg_colour_simple "info" "server: ${yellow}$tag${white} at address: ${yellow}$pubIp${reset}"
  printf "\n%s"
  generic_msg_colour_simple "info-indented" "building:    server specific paths"                                        && sleep ${STEP_PAUSE}
  generic_msg_colour_simple "info-indented" "writing-to:  server specific paths"
  printf "%s\n" "${red}"
