...
if [[ "${pod_stop_dse_fail}" == "true" ]]; then
  printf "%s\n"
  pod_generic_display_msgColourSimple "info-bold" "--> ${red}Dse stop errors report:"                                                               && sleep "${STEP_PAUSE}"
  printf "%s\n"
  ...
  printf "%s\n"
  pod_generic_display_msgColourSimple "error" "Could not stop DSE on this server"
else
