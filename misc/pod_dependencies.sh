...
source ${podHomePath}/pods/pod_/prepare/GENERIC_prepare_misc.bash;
# identify local os
...
if [[ "${os}" == "Mac" ]]; then
  ...
  GENERIC_prepare_display_stageTimeCount "${STAGE_PAUSE}" "Proceeding to next STAGE...";
  ...
  if [[ $brewList == *"grep"* ]]; then
    ...
    printf "%s\t" "$ brew uninstall grep && brew install grep --with-default-names (takes ~ 2m)";
    ...
    printf "%s\t" "$ brew install grep --with-default-names     (takes ~ 2m)";
    brew install grep --with-default-names > /dev/null 2>&1;
    printf "%s\n" "${tick}";
  fi;
...
# if not a Mac - just add to bash_profile
else
