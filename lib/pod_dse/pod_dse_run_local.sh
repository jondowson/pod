...
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "export CASSANDRA_LOG_DIR=" "dummy"
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "export TOMCAT_LOGS=" "dummy"
...
# search for and remove any lines starting with:
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "-Djna.tmpdir=" "dummy"
...
pod_generic_strings_sedStringManipulation "editAfterSubstring" "${file}" "dc="   "${dc}"
pod_generic_strings_sedStringManipulation "editAfterSubstring" "${file}" "rack=" "${rack}"
}
...
# cluster_name:
pod_generic_strings_sedStringManipulation "editAfterSubstring" "${file}" "cluster_name:" "'${CLUSTER_NAME}'"

# num_tokens:
# allocate_tokens_for_local_replication_factor: 3 (uncomment for vnodes)
if [[ "${VNODES}" == "false" ]]; then
  pod_generic_strings_sedStringManipulation "removeHashAndLeadingWhitespace" "${file}" "initial_token:" "dummy"
  pod_generic_strings_sedStringManipulation "editAfterSubstring"             "${file}" "initial_token:" "${token}"
  pod_generic_strings_sedStringManipulation "hashCommentOutMatchingLine"     "${file}" "num_tokens:" "dummy"
  pod_generic_strings_sedStringManipulation "hashCommentOutMatchingLine"     "${file}" "allocate_tokens_for_local_replication_factor:" "dummy"
 else
  pod_generic_strings_sedStringManipulation "removeHashAndLeadingWhitespace" "${file}" "num_tokens:" "dummy"
  pod_generic_strings_sedStringManipulation "editAfterSubstring"             "${file}" "num_tokens:" "${VNODES}"
  pod_generic_strings_sedStringManipulation "removeHashAndLeadingWhitespace" "${file}" "allocate_tokens_for_local_replication_factor:" "dummy"
  pod_generic_strings_sedStringManipulation "hashCommentOutMatchingLine"     "${file}" "initial_token:" "dummy"
fi

# hints_directory:
pod_generic_strings_sedStringManipulation "editAfterSubstringPathFriendly" "${file}" "hints_directory:" "${hints_directory}"

# commitlog_directory:
pod_generic_strings_sedStringManipulation "editAfterSubstringPathFriendly" "${file}" "commitlog_directory:" "${commitlog_directory}"

# cdc_raw_directory:
pod_generic_strings_sedStringManipulation "editAfterSubstringPathFriendly" "${file}" "cdc_raw_directory:" "${cdc_raw_directory}"

# saved_caches_directory:
pod_generic_strings_sedStringManipulation "editAfterSubstringPathFriendly" "${file}" "saved_caches_directory:" "${saved_caches_directory}"

# endpoint_snitch: (nearly always 'GossipingPropertyFileSnitch')
pod_generic_strings_sedStringManipulation "editAfterSubstring" "${file}" "endpoint_snitch:" "${ENDPOINT_SNITCH}"
}
...
# listen_address:
pod_generic_strings_sedStringManipulation "editAfterSubstring" "${file}" "listen_address:" "${listen_address}"

# rpc_address:
pod_generic_strings_sedStringManipulation "editAfterSubstring" "${file}" "rpc_address:" "${rpc_address}"
}
...
# search for and remove any lines starting with:
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "export SPARK_WORKER_LOG_DIR=" "dummy"
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "export SPARK_MASTER_LOG_DIR=" "dummy"
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "export SPARK_WORKER_DIR=" "dummy"
pod_generic_strings_sedStringManipulation "searchFromLineStartAndRemoveEntireLine" ${file} "export SPARK_LOCAL_DIRS=" "dummy"
...
if [[ ${remote_os} == *"Mac"* ]]; then
  ...
  pod_generic_strings_sedStringManipulation "searchAndReplaceStringGlobal" "${tmp_working_folder}launch-pod" "bin/bash" "${hashBang}"
  pod_generic_strings_sedStringManipulation "searchAndReplaceStringGlobal" "${tmp_working_folder}lib/pod_dse/pod_dse_script_launch_remote.sh" "bin/bash" "${hashBang}"
  pod_generic_strings_sedStringManipulation "searchAndReplaceStringGlobal" "${tmp_working_folder}lib/pod_dse/pod_dse_run_remote.sh" "bin/bash" "${hashBang}"
elif [[ ${remote_os} == "Ubuntu" ]]; then
  hashBang="bin/bash"
  pod_generic_strings_sedStringManipulation "searchAndReplaceStringGlobal" "${tmp_working_folder}launch-pod" "usr/local/bin/bash" "${hashBang}"
  pod_generic_strings_sedStringManipulation "searchAndReplaceStringGlobal" "${tmp_working_folder}lib/pod_dse/pod_dse_script_launch_remote.sh" "usr/local/bin/bash" "${hashBang}"
  pod_generic_strings_sedStringManipulation "searchAndReplaceStringGlobal" "${tmp_working_folder}lib/pod_dse/pod_dse_run_remote.sh" "usr/local/bin/bash" "${hashBang}"
