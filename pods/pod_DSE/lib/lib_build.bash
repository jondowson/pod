...
if [[ "${localServer}" == "true" ]] && [[ "${LOCAL_TARGET_FOLDER}" != "${target_folder}" ]]; then
  printf "%s\n" "bash_path_string=${LOCAL_TARGET_FOLDER}POD_SOFTWARE/POD/pod" >> "${tmp_suitcase_file_path}"
else
  printf "%s\n" "bash_path_string=${target_folder}POD_SOFTWARE/POD/pod" >> "${tmp_suitcase_file_path}"
fi
}
