# about:         distribute + configure pod_DSE-OPSCENTER to server(s)
...
# pod_DSE-OPSCENTER makes use of 2 user defined files and has 6 STAGES.
...
declare -A start_opscenter_error_array
...
if [[ "${clusterstateFlag}" == "true" ]]; then
  ...
  if [[ "${CLUSTER_STATE}" == "restart" ]]; then
    prepare_generic_display_stageCount        "Restarting dse opscenter" "2" "3"
    prepare_generic_display_msgColourSimple   "TASK==>"  "TASK: Restarting opscenter daemon"
    task_rollingStop
    task_rollingStart
    task_rollingStart_report
  else
    prepare_generic_display_stageCount        "Stopping dse opscenter" "2" "3"
