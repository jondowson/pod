...
# -----
  ...
  generic_msg_colour_simple "info-indented" "building:    server specific 'pod'"                                        && sleep ${STEP_PAUSE}
  ...
  if [ $VB == true ]; then generic_msg_colour_simple "info-indented" "editing:     'build_folder_path' in 'setup_pod_remote.sh'"; fi
  generic_sed_string_manipulation "editAfterSubstring" "${tmp_build_folder}misc/setup_pod_remote.sh" "build_folder_path=" "\"${target_folder}pod/builds/pod_dse/${BUILD_FOLDER}/\""
...
# -----
  ...
  generic_msg_colour_simple "info-indented" "sending:     server specific 'pod'"
  printf "%s\n" "${red}"
