...
# -----
  ...
  if [[ "${generate_keys}" == "true" ]]; then
    ...
    prepare_generic_display_msgColourSimple "INFO-->" "retrieving encryption keys:     from server 1 to local resources folder"
    if [[ "${id}" == "1" ]]; then
      scp -i ${sshKey} ${user}@${pubIp}:${system_key_directory}* ${resources_folder}
    fi
    prepare_generic_display_msgColourSimple "INFO-->" "copying encryption keys:     to remote keys folder"
