...
logfile="logback-spark.xml"
if [ "$LOG_FILE_FOR_SHELL" = "logback-spark-shell.xml" ]; then
    logfile="$LOG_FILE_FOR_SHELL"
fi
SPARK_SUBMIT_LOGBACK_CONF_FILE="${SPARK_SUBMIT_LOGBACK_CONF_FILE:-"$SPARK_CONF_DIR/$logfile"}"
